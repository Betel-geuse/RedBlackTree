...
private:
	...
	void insert(RBTNode<T>*& root, RBTNode<T>* node);// ¬ставл€ем узел, внутренний интерфейс
	void InsertFixUp(RBTNode<T>*& root, RBTNode<T>* node);
	void destory(RBTNode<T>*& node);

	void remove(RBTNode<T>*& root, RBTNode<T>* node);// ”далить узел по ключу
...
template<class T>		// онструктор
RBTree<T>::RBTree() :root(NULL) {
	root = nullptr;
}
template<class T>		// ƒеструктор
RBTree<T>::~RBTree() {
	destory(root);
}
template<class T>		
...
template<class T>		//ѕравый поворот
...
void RBTree<T>::remove(RBTNode<T>*& root, RBTNode<T>* node)
{
	...
	//  огда в удаленном узле только левый (правый) узел пуст, найдем дочерний узел удаленного узла
	if (node->left != NULL)
		child = node->left;
	else
		child = node->right;
...
template<class T>		// ¬ывод детальной информации о двоичном дереве
void RBTree<T>::print() {
	if (root == NULL)
		cout << "empty RBtree\n";
	else
...
void RBTree<T>::print(RBTNode<T>* node)const {
	if (node == NULL)
		return;
	if (node->parent == NULL)
		std::cout << node->key << "(" << node->color << ") is root" << endl;
...
template<class T>
void RBTree<T>::preOrder(RBTNode<T>* tree)const { //корень-левое-правое
	if (tree != NULL) {
		cout << tree->key << " ";
		preOrder(tree->left);
		preOrder(tree->right);
...
template<class T>		// ќбход дерева RB
...
template<class T>
void RBTree<T>::inOrder(RBTNode<T>* tree)const {//левое-корень-правое
	if (tree != NULL) {
		inOrder(tree->left);
		cout << tree->key << " ";
		inOrder(tree->right);
...
template<class T>
void RBTree<T>::postOrder(RBTNode<T>* tree)const {//левое-правое-корень
	if (tree != NULL) {
		postOrder(tree->left);
